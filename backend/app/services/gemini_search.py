...
def _build_basic_summary(item: dict) -> str:
    ...
    overview = fields.get("서비스 요약") or fields.get("개요") or fields.get("지원 내용") or item.get("title", "정보")
    target = fields.get("지원 대상") or fields.get("대상 특성")
    apply_method = fields.get("신청 방법")
    contact = fields.get("문의") or fields.get("문의 전화") or fields.get("대표 문의처")

    sections = ["### 🎁 어떤 혜택(행사)인가요?", f"{overview}"]
    sections.extend(["", "### 🙋‍♀️ 누가 받을 수 있나요?"])
    if target:
        sections.append(f"- {target}")
    else:
        sections.append("ℹ️ 원문에 자세한 안내가 없어 확인이 어려워요.")
    sections.extend(["", "### 📅 언제까지인가요?"])
    sections.append("📌 원문에 신청 기간이 자세히 적혀 있지 않아요.")
    sections.extend(["", "### 📝 어떻게 신청하나요?"])
    if apply_method:
        sections.append(f"- {apply_method}")
    else:
        sections.append("🧭 신청 방법은 문의처에서 확인해 주세요.")
    if contact:
        sections.extend(["", f"📌 문의: **{contact}**"])
...
아래에 주어지는 여러 복지/행사 항목을 각각 위의 [요약 규칙]과 [출력 포맷]에 따라 요약하세요.

## 중요: summary 필드 형식 규칙
- 반드시 [출력 포맷]에 정의된 4개 섹션 제목(### 🎁, ### 🙋‍♀️, ### 📅, ### 📝)을 그대로 사용하세요.
- 모든 내용을 하나의 단락으로 합치지 마세요. 반드시 섹션별로 나눠서 작성하세요.
- 줄바꿈은 \\n으로 표현하세요.
